...
#. Get the Python 3 porting status using ``py3query`` dnf plugin. Use ``-o`` option to write the output directly to ``fedora.json``::
    ...
    $ dnf-3 --repo=rawhide --repo=rawhide-source --qrepo=rawhide py3query --refresh --installroot=/tmp/empty-install-root --misnamed=data/misnamed-old.yaml -o data/fedora.json
